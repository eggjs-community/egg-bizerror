...
module.exports = class BizErrorHanlder {
  ...
  getResponseInfo(ctx, error) {
    ...
    if (!codeConfig) {
      ...
      codeConfig = errorCode.SYSTEM_EXCEPTION;
    }
    ...
    Object.keys(codeConfig).forEach(key => {
      const target = this.keyword.includes(key)
        ? option
        : responseInfo;

      target[key] = codeConfig[key];
    });
  ...
  responseError(ctx, error) {
    ...
    if (responseInfo === false || config.disableDefaultHandler === true) return;
  ...
  jsonp(ctx, error, responseInfo) {
    ctx.createJsonpBody
      ? ctx.createJsonpBody(responseInfo)
      : this.json(ctx, error, responseInfo);
  }
};
